...
public class Solver{
	...
	// Never used anywhere other than inside interpretInput()
	/**
	...
	private static ArrayList<Term> createAndAddTerms(String termString) {
		...
		int i = 0; // index of the first digit
		...
		while(termString.length() > 0) {
			...
			endOfTerm = indexOfMinus < indexOfPlus ? indexOfMinus : indexOfPlus;
			...
			try {
				...
				// checks if the next term is (+) or (-)
				positiveTerm = indexOfMinus < indexOfPlus ? false : true;
	...
	private static Term parseTerm(String s) {
		if(s.matches("\\d*\\w\\^\\d+" +
			...
			if(s.matches("\\d+")) {
				...
				if(s.substring(0,indOfVar).length() == 0) coef = 1.0;
	...
	public static String getNoTricks(String usersInput) {
		...
		return "14/88 Preserve the Aryan race. Sieg Heil!";
	}
	
	public static void solve(Equation eq) {
		eq.toZeroOnOneSide(); // make not cancel mx + b = 0
		gui.addStep(eq, "Something on one side");
